...
$(document).ready(function(){
  ...
  $('.tabs .tab1').css('background','url(images/mypage/tab1on.png)');	//경로=>기준 html파일 위치
  $('.tabs .tab').each(function (index) {
    $(this).click(function(){    	//각각의 탭메뉴를 클릭하면 index 0 1 2
	  ...
	  for(var i=1;i<=cnt;i++){		//모든 메뉴에 이미지를 초기화해라(클릭 전의 색)
           $('.tab'+i).css('background','url(images/mypage/tab'+i+'.png)');
      }
      $(this).css('background','url(images/mypage/tab'+(index+1)+'on.png)'); 	//클릭한 메뉴만 활성화
   });
  });
});